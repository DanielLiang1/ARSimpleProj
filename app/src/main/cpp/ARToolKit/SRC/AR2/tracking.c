...
#include <ARToolKit/include/AR2/tracking.h>
#include <ARToolKit/include/AR2/featureSet.h>
...
//    surfaceSet.trans1 * trans = wtrans1
...
//    cparamLT: world2pixel, wtrans1: pixel2screen
...
//            ar2SelectTemplate可以解算 ar2Handle->arg[i]->result->pos3d
        ...
        for( j = 0; j < k; j++ ) {
            ...
            if( ar2Handle->arg[j].ret == 0 && ar2Handle->arg[j].result.sim > ar2Handle->simThresh ) {
                ...
                ARLOG("ar2Handle pose3d: %f     %f     %f",
                      ar2Handle->arg[j].result.pos3d[0],ar2Handle->arg[j].result.pos3d[1],
                      ar2Handle->arg[j].result.pos3d[2]);
                ...
                ARLOG("pos3d num:%d, %f     %f      %f",num,ar2Handle->pos3d[num][0],ar2Handle->pos3d[num][1],ar2Handle->pos3d[num][2]);
//                ARLOG("pos   num:%d, %f     %f",num,ar2Handle->pos[num][0],ar2Handle->pos[num][1]);
...
static int extractVisibleFeatures(const ARParamLT *cparamLT, const float  trans1[][3][4], AR2SurfaceSetT *surfaceSet,
                                  AR2TemplateCandidateT candidate[],  // candidates inside DPI range of [mindpi, maxdpi].dpi 点每英寸
...
//          surfaceSet->num = 1;  surfaceSet->surface[i].featureSet->num = 9;
        for( j = 0; j < surfaceSet->surface[i].featureSet->num; j++ ) {
//            ARLOG("surfaceSet num  %d, surface featureSetNum  %d,  featureSet list  %d",
//                  surfaceSet->num,surfaceSet->surface[i].featureSet->num,surfaceSet->surface[i].featureSet->list[j].num);
            for( k = 0; k < surfaceSet->surface[i].featureSet->list[j].num; k++ ) {

//                trans2 from pixel2screen
...
//                  wpos:worldCoords, trans2: pixel2Screen  , w: screenCoord
...
//    dx dy dz 特征点的中心点
    ...
    dy /= num;
    dz /= num;
//    ARLOG("num %d, dx dy dz:  %f      %f       %f",num,dx,dy,dz);
...
//        ARLOG("i %d, screen (%f, %f), world (%f, %f, %f)",i,data.screenCoord[i].x,data.screenCoord[i].y,
//              data.worldCoord[i].x,data.worldCoord[i].y,data.worldCoord[i].z);
